...
func TestNewRequestForPrivateApp(t *testing.T) {
	...
	testClient := NewClient(testApp, "fooshop", "", WithVersion(testApiVersion))
	...
	if username != testApp.ApiKey {
		t.Errorf("NewRequestPrivateApp() Username = %v, expected %v", username, testApp.ApiKey)
	}

	if password != testApp.Password {
		t.Errorf("NewRequestPrivateApp() Password = %v, expected %v", password, testApp.Password)
	}
